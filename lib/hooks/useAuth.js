...
import { onAuth, signIn, signUp, signOut, signInWithGoogle, resetPassword } from "@/lib/firebase/auth";
  ...
  const loginWithGoogle = useCallback(async () => {
    try {
      ...
      if (code === "auth/popup-closed-by-user" || code === "auth/cancelled-popup-request") {
        ...
        msg = "Tarayıcı pop-up'ı engelledi. Lütfen izin verin ve tekrar deneyin.";
      } else if (code === "auth/unauthorized-domain") {
        msg = "Bu domain Firebase'de yetkili değil. Lütfen yönetici ile iletişime geçin.";
      } else if (code === "auth/network-request-failed") {
        msg = "Ağ bağlantısı hatası. İnternet bağlantınızı kontrol edin.";
      } else if (code) {
        msg = `Google girişi başarısız: ${code}`;
      }
