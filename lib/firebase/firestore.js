...
// ═══════════════════════════════════════════════════
//   SEED BLOG POSTS (one-time migration)
// ═══════════════════════════════════════════════════

export async function seedBlogPosts(postsArray) {
  const batch = writeBatch(db);
  for (const p of postsArray) {
    // eslint-disable-next-line no-unused-vars
    const { id: _numericId, ...postData } = p;
    const ref = doc(collection(db, "blog"));
    batch.set(ref, {
      ...postData,
      published: postData.published ?? true,
      author: postData.author || "GIRLBOSS",
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
    });
  }
...
export function onDiscountsSnapshot(callback, onError) {
  return onSnapshot(
    query(collection(db, "discounts"), orderBy("createdAt", "desc")),
    (snap) => callback(snap.docs.map((d) => ({ ...d.data(), id: d.id }))),
    onError ?? ((err) => console.error("[Firestore] discounts snapshot error:", err))
  );
}
