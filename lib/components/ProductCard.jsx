...
import { ShoppingBag, Star, Heart, ArrowRight, Sparkles, Flame, Crown } from "lucide-react";
  ...
  return (
    ...
    >
      <div className="bg-white rounded-3xl overflow-hidden shadow-[0_2px_20px_rgba(0,0,0,0.04)] hover:shadow-[0_16px_48px_rgba(183,110,121,0.14)] transition-all duration-500 border border-[#f0e8e4] hover:border-[#fecdd3] relative">
        ...
        >
          <Heart size={14} className="text-[#b76e79]" fill={fav ? "#b76e79" : "none"} />
        </motion.button>
          ...
          <AnimatePresence>
            {isHovered && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 16 }}
                ...
                <Link href={`/urunler/${product.slug}`} className="w-full py-2.5 bg-white/95 backdrop-blur-sm text-[#2d2d2d] text-[11px] tracking-wider font-semibold uppercase rounded-xl flex items-center justify-center gap-2 hover:bg-white transition-colors shadow-sm">
        ...
        <div style={{ padding: '1.25rem' }}>
          ...
          <div className="flex items-center justify-between mb-1">
            <Link href={`/urunler/${product.slug}`} className="text-[15px] font-bold text-[#2d2d2d] truncate pr-2 hover:text-[#b76e79] transition-colors">
              {product.name}
            </Link>
          ...
          {/* Price & CTA */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '0.75rem', borderTop: '1px solid #f3efed', gap: '0.5rem' }}>
            <span style={{ fontSize: '1rem', fontWeight: '700', color: '#2d2d2d', flexShrink: 0 }}>₺{product.price}</span>
            <GBButton
