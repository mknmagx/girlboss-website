...
import { getProductBySlug, getProducts, getProductReviews, getSettings } from "@/lib/firebase/firestore";
...
export default function ProductDetailPage({ params }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      setRelated(allProds.filter((x) => x.id !== p.id && x.active !== false).slice(0, 3));
      setAllReviews(reviews.filter((r) => r.approved !== false));
  ...
  /* alias for JSX references */
  const ALL_REVIEWS = allReviews;
          ...
          >
            ...
            <div className="hero-fade-1 flex flex-col justify-center">
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                >
                  {product.reviewCount ?? ALL_REVIEWS.length} değerlendirme →
                </button>
              </div>
        ...
        <div className="gb-container">
          ...
          >
            <div className="text-center shrink-0">
              ...
              <p className="text-sm text-[#737373]">{product.reviewCount ?? ALL_REVIEWS.length} değerlendirme</p>
            </div>
          ...
          >
            {[
              { key: "all", label: "Tümü", count: ALL_REVIEWS.length },
              { key: "5", label: "5 Yıldız", count: ALL_REVIEWS.filter((r) => r.rating === 5).length },
              { key: "4", label: "4 Yıldız", count: ALL_REVIEWS.filter((r) => r.rating === 4).length },
              { key: "low", label: "3 ve altı", count: ALL_REVIEWS.filter((r) => r.rating <= 3).length },
              { key: "with-images", label: "Görselli", count: ALL_REVIEWS.filter((r) => r.images?.length > 0).length, icon: true },
